...
export default function FAQs() {
  return (
    <div className="flex flex-col justify-center w-full max-w-4xl min-h-[80vh]" id="faq">
      <Heading index={"07"} title={"FAQ's"} />

      <div className="my-6 space-y-4">
        {faqsData.map((faq, index) => (
          <details key={index} className="group [&_summary::-webkit-details-marker]:hidden">
            <summary className="flex cursor-pointer items-center justify-between gap-1.5 rounded-lg text-white p-4 glassmorphism font-mono text-2xl ">
            ...
            <p className="px-4 mt-4 leading-relaxed text-white text-lg">{faq.answer}</p>
          </details>
        ))}
      </div>
    </div>
